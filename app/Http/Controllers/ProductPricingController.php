...
class ProductPricingController extends Controller
{
    ...
    public function editProductPricing(Request $request, $id){
        $edits = ProductPricing::where('id', $id)->first();
        $categories = ProductPricing::all();
        return view("admin.categories", ['edit' => $edits, 'categories' => $categories]);
    }

    public function deleteProductPricing(Request $request)
    {
        // Access the 'id' from the request
        $id = $request->input('id');  // or $request->get('id')
        // Now you can use $id to find and delete the category
        $category = ProductPricing::find($id);
        if ($category) {
     ...
     function store(Request $request){
        ...
        // Verify the HMAC signature only if we're editing an existing product pricing
        ...
        $db = ($postId)? ProductPricing::find($postId) : new ProductPricing();
    ...
    public function ProductPricing(){
        ...
        return view("admin.product_pricing", ['deleted'=> true, 'product_pricing' => $product_pricing]);
    }
}
