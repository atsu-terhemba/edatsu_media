    ...
    return (
        <div
            ...
            style={{
                ...
                transition: 'all 300ms ease', transform: hovered ? 'translateY(-4px)' : 'translateY(0)',
                display: 'flex', alignItems: 'center', gap: '20px',
            }}
        >
            <div style={{
                width: '48px', height: '48px', borderRadius: '9999px', flexShrink: 0, backgroundColor: '#000',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                transition: 'transform 300ms ease', transform: hovered ? 'scale(1.1)' : 'scale(1)',
            }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {item.description && (
                    <div style={{ fontSize: '14px', color: '#6e6e73', lineHeight: 1.5, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.description}</div>
                )}
            </div>
            <div style={{ fontSize: '12px', color: '#86868b', flexShrink: 0, textAlign: 'right', minWidth: '80px' }}>
        ...
        <Modal show={show} onHide={onHide} size="lg" centered>
            <div style={{ borderRadius: '16px', overflow: 'hidden' }}>
                <div style={{ padding: '24px 32px 16px', borderBottom: '1px solid #f0f0f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <button onClick={onHide} style={{ width: '32px', height: '32px', borderRadius: '9999px', border: '1px solid #e5e5e5', backgroundColor: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all 150ms ease' }}>
...
export default function ManageTags({ tags, edit }) {
    ...
    useState(() => {
        if (edit) { setFormData({ name: edit.name || '', slug: edit.slug || '', description: edit.description || '' }); setShowEditModal(true); setEditingTag(edit); }
    }, [edit]);
    ...
    const resetForm = () => { setFormData({ name: '', slug: '', description: '' }); setEditingTag(null); };

    const handleSubmit = async (e) => {
        e.preventDefault(); setIsLoading(true);
        try {
            const url = editingTag ? `/edit-tag/${editingTag.id}` : '/admin-store-tag';
            const response = await axios.post(url, { ...formData, post_id: editingTag?.id || null });
            if (response.data.success) { showAlertMsg(response.data.message, 'success'); resetForm(); setShowModal(false); setShowEditModal(false); router.visit(route('admin.tag'), { preserveState: false }); }
    ...
    const handleEdit = (tag) => { setEditingTag(tag); setFormData({ name: tag.name, slug: tag.slug, description: tag.description }); setShowEditModal(true); };
    const handleDelete = async (tagId) => {
        if (!confirm('Are you sure you want to delete this tag?')) return;
        setIsLoading(true);
        try {
            const response = await axios.post('/delete-tag', { id: tagId });
            if (response.data.success) { showAlertMsg(response.data.message, 'success'); router.visit(route('admin.tag'), { preserveState: false }); }
            else { showAlertMsg(response.data.message || 'An error occurred', 'danger'); }
        } catch (error) { showAlertMsg(error.response?.data?.message || 'An error occurred while deleting', 'danger'); }
        finally { setIsLoading(false); }
    };

    const filtered = tags.filter(t => t.name.toLowerCase().includes(searchQuery.toLowerCase()) || t.slug.toLowerCase().includes(searchQuery.toLowerCase()));
        ...
        <form onSubmit={handleSubmit}>
            <div style={{ padding: '32px' }}>
                <div style={{ marginBottom: '24px' }}>
                    ...
                    <input type="text" name="name" value={formData.name} onChange={handleInputChange} required placeholder="Enter tag name" style={inputStyle} onFocus={e => e.target.style.borderColor = '#000'} onBlur={e => e.target.style.borderColor = '#e5e5e5'} />
                </div>
                <div style={{ marginBottom: '24px' }}>
                    <label style={labelStyle}>Slug *</label>
                    <input type="text" name="slug" value={formData.slug} onChange={handleInputChange} required placeholder="tag-slug" style={{ ...inputStyle, fontFamily: 'monospace', fontSize: '13px' }} onFocus={e => e.target.style.borderColor = '#000'} onBlur={e => e.target.style.borderColor = '#e5e5e5'} />
                    <div style={{ fontSize: '12px', color: '#86868b', marginTop: '6px' }}>URL-friendly version of the name (auto-generated)</div>
                </div>
                <div style={{ marginBottom: '8px' }}>
                    <label style={labelStyle}>Description *</label>
                    <textarea name="description" value={formData.description} onChange={handleInputChange} required rows={3} placeholder="Enter tag description" style={{ ...inputStyle, resize: 'vertical', minHeight: '80px' }} onFocus={e => e.target.style.borderColor = '#000'} onBlur={e => e.target.style.borderColor = '#e5e5e5'} />
                </div>
            </div>
    ...
    return (
        <AuthenticatedLayout>
                    ...
                    <Col md={9}>
                        <div style={{ padding: '8px 0' }}>
                            ...
                            {alert.show && (
                                <div style={{ padding: '14px 20px', borderRadius: '12px', backgroundColor: alert.type === 'success' ? '#f0fdf4' : '#fef2f2', border: `1px solid ${alert.type === 'success' ? '#bbf7d0' : '#fecaca'}`, color: alert.type === 'success' ? '#166534' : '#991b1b', fontSize: '14px', marginBottom: '24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                        <span className="material-symbols-outlined" style={{ fontSize: '20px' }}>{alert.type === 'success' ? 'check_circle' : 'error'}</span>{alert.message}
                                    </div>
                                    <button onClick={() => setAlert({ show: false, message: '', type: 'success' })} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px' }}>
                                        <span className="material-symbols-outlined" style={{ fontSize: '18px', color: 'inherit' }}>close</span>
                                    </button>
                                </div>
                            )}
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
                                <div style={{ fontSize: '14px', color: '#86868b' }}>{filtered.length} {filtered.length === 1 ? 'tag' : 'tags'}</div>
                                <div style={{ position: 'relative', width: '280px' }}>
                                    ...
                                    <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search tags..." style={{ ...inputStyle, paddingLeft: '40px', borderColor: '#f0f0f0', backgroundColor: '#f5f5f7' }}
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                {filtered.length > 0 ? filtered.map(tag => <ItemCard key={tag.id} item={tag} icon="sell" onEdit={handleEdit} onDelete={handleDelete} />) : (
                                    <div style={{ padding: '64px 32px', borderRadius: '16px', backgroundColor: '#f5f5f7', textAlign: 'center' }}>
                                        <span className="material-symbols-outlined" style={{ fontSize: '48px', color: '#d1d1d6', display: 'block', marginBottom: '16px' }}>sell</span>
                                        <div style={{ fontSize: '16px', fontWeight: 600, color: '#000', marginBottom: '8px' }}>{searchQuery ? 'No matching tags' : 'No tags yet'}</div>
                                        <div style={{ fontSize: '14px', color: '#86868b' }}>{searchQuery ? 'Try adjusting your search query.' : 'Create your first tag to get started.'}</div>
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            <StyledModal show={showModal} onHide={() => setShowModal(false)} eyebrow="Tag" title="Add New Tag"><ItemForm onCancel={() => setShowModal(false)} submitLabel="Save Tag" /></StyledModal>
            <StyledModal show={showEditModal} onHide={() => setShowEditModal(false)} eyebrow="Tag" title="Edit Tag"><ItemForm onCancel={() => setShowEditModal(false)} submitLabel="Update Tag" /></StyledModal>
        </AuthenticatedLayout>
    );
}
