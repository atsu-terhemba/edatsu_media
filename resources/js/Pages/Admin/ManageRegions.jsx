...
export default function ManageRegions({ regions, edit }) {
    ...
    useState(() => { if (edit) { setFormData({ name: edit.name || '', slug: edit.slug || '', description: edit.description || '' }); setShowEditModal(true); setEditingRegion(edit); } }, [edit]);
    ...
    const resetForm = () => { setFormData({ name: '', slug: '', description: '' }); setEditingRegion(null); };

    const handleSubmit = async (e) => {
        e.preventDefault(); setIsLoading(true);
        try {
            const url = editingRegion ? `/edit-region/${editingRegion.id}` : '/admin-store-region';
            const response = await axios.post(url, { ...formData, post_id: editingRegion?.id || null });
            if (response.data.success) { showAlertMsg(response.data.message, 'success'); resetForm(); setShowModal(false); setShowEditModal(false); router.visit(route('admin.regions'), { preserveState: false }); }
    ...
    const handleEdit = (region) => { setEditingRegion(region); setFormData({ name: region.name, slug: region.slug, description: region.description }); setShowEditModal(true); };
    const handleDelete = async (regionId) => {
        if (!confirm('Are you sure you want to delete this region?')) return;
        setIsLoading(true);
        try {
            const response = await axios.post('/delete-region', { id: regionId });
            if (response.data.success) { showAlertMsg(response.data.message, 'success'); router.visit(route('admin.regions'), { preserveState: false }); }
            else { showAlertMsg(response.data.message || 'An error occurred', 'danger'); }
        } catch (error) { showAlertMsg(error.response?.data?.message || 'An error occurred while deleting', 'danger'); }
        finally { setIsLoading(false); }
    };

    const filtered = regions.filter(r => r.name.toLowerCase().includes(searchQuery.toLowerCase()) || r.slug.toLowerCase().includes(searchQuery.toLowerCase()));

    const ItemForm = ({ onCancel, submitLabel }) => (
        <form onSubmit={handleSubmit}>
            <div style={{ padding: '32px' }}>
                <div style={{ marginBottom: '24px' }}><label style={labelStyle}>Region Name *</label><input type="text" name="name" value={formData.name} onChange={handleInputChange} required placeholder="Enter region name" style={inputStyle} onFocus={e => e.target.style.borderColor = '#000'} onBlur={e => e.target.style.borderColor = '#e5e5e5'} /></div>
                <div style={{ marginBottom: '24px' }}><label style={labelStyle}>Slug *</label><input type="text" name="slug" value={formData.slug} onChange={handleInputChange} required placeholder="region-slug" style={{ ...inputStyle, fontFamily: 'monospace', fontSize: '13px' }} onFocus={e => e.target.style.borderColor = '#000'} onBlur={e => e.target.style.borderColor = '#e5e5e5'} /><div style={{ fontSize: '12px', color: '#86868b', marginTop: '6px' }}>URL-friendly version of the name (auto-generated)</div></div>
                <div style={{ marginBottom: '8px' }}><label style={labelStyle}>Description *</label><textarea name="description" value={formData.description} onChange={handleInputChange} required rows={3} placeholder="Enter region description" style={{ ...inputStyle, resize: 'vertical', minHeight: '80px' }} onFocus={e => e.target.style.borderColor = '#000'} onBlur={e => e.target.style.borderColor = '#e5e5e5'} /></div>
            </div>
    ...
    return (
        <AuthenticatedLayout>
                ...
                <Row>
                    ...
                    <Col md={9}>
                        <div style={{ padding: '8px 0' }}>
                            <SectionHeader eyebrow="General" title="Regions" action={
                                <button onClick={() => { resetForm(); setShowModal(true); }} style={{ padding: '10px 24px', borderRadius: '9999px', border: 'none', backgroundColor: '#000', color: '#fff', fontSize: '14px', fontWeight: 500, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: '6px', transition: 'background-color 150ms ease' }}
                                    ...
                                    <span className="material-symbols-outlined" style={{ fontSize: '18px' }}>add</span>Add Region
                                </button>
                            } />
                            {alert.show && (
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
                                <div style={{ fontSize: '14px', color: '#86868b' }}>{filtered.length} {filtered.length === 1 ? 'region' : 'regions'}</div>
                                <div style={{ position: 'relative', width: '280px' }}>
                                    ...
                                    <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search regions..." style={{ ...inputStyle, paddingLeft: '40px', borderColor: '#f0f0f0', backgroundColor: '#f5f5f7' }}
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                {filtered.length > 0 ? filtered.map(region => <ItemCard key={region.id} item={region} icon="map" onEdit={handleEdit} onDelete={handleDelete} />) : (
                                    <div style={{ padding: '64px 32px', borderRadius: '16px', backgroundColor: '#f5f5f7', textAlign: 'center' }}>
                                        <span className="material-symbols-outlined" style={{ fontSize: '48px', color: '#d1d1d6', display: 'block', marginBottom: '16px' }}>map</span>
                                        <div style={{ fontSize: '16px', fontWeight: 600, color: '#000', marginBottom: '8px' }}>{searchQuery ? 'No matching regions' : 'No regions yet'}</div>
                                        <div style={{ fontSize: '14px', color: '#86868b' }}>{searchQuery ? 'Try adjusting your search query.' : 'Create your first region to get started.'}</div>
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            <StyledModal show={showModal} onHide={() => setShowModal(false)} eyebrow="Region" title="Add New Region"><ItemForm onCancel={() => setShowModal(false)} submitLabel="Save Region" /></StyledModal>
            <StyledModal show={showEditModal} onHide={() => setShowEditModal(false)} eyebrow="Region" title="Edit Region"><ItemForm onCancel={() => setShowEditModal(false)} submitLabel="Update Region" /></StyledModal>
        </AuthenticatedLayout>
    );
}
